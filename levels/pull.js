...
const hint = `'git pull' takes two arguments. The first is the remote that you wish to pull from. The second is the branch of the remote you wish to pull. For example, a the most common way this command is used is 'git pull origin master', since 'origin' is the default remote name. 

Fun note: Git pull is actually shorthand for two commands! It will run 'git fetch' which fetches the code from the remote repository, and then 'git merge' which merges the code into the local branch. We will look more at these commands later!
`

function setup() {
  return level.repoInit()
  .then(repo => Git.Remote.create(repo, 'dummy-remote', 'https://github.com/Kmacpher/dummy-remote'))
  .then(() => console.log('The challenge has been set up!'));
}
